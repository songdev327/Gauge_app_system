...
export default function RequestGaugeForm() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            // ---------------------------------------------------------
            // 🔍 **ขั้นตอนสำคัญ: เช็ค Doc No. ซ้ำ**
            // ---------------------------------------------------------
            const check = await axios.get(`${config.api_path}/gauge-request/check-docno`, {
                params: { docNo: formData.docNo }
            });
    
            if (check.data.exists) {
                Swal.fire({
                    title: "Doc No. ซ้ำ!",
                    text: "หมายเลขเอกสารนี้ถูกใช้งานแล้ว ไม่สามารถยืม Gauge ได้",
                    icon: "error"
                });
                return; // ❌ หยุดการทำงาน ไม่ส่งข้อมูลเข้า DB
            }
    
            // ❗ เช็คผู้ขอ/ผู้จ่าย/ผู้รับ
